...
class CartridgeHeaderSectionTests: XCTestCase {
    func testSections() {
        let sections = (0x00..<0x50).compactMap { Cartridge.Header.Section(rawValue: $0) }
        XCTAssertEqual(sections.count, Cartridge.Header.Section.allSections.count)
    }
    
    func testRanges() {
        for section in Cartridge.Header.Section.allSections {
            switch section {
            case .boot:
